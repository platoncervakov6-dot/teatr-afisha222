...
import { dedupeByKey, mapCategoryRu, normalizeText } from "../utils.js";
...
export async function aggregateAll({ city = "moscow", noCache = false } = {}) {
  ...
  // Дедуп по названию + площадке + дате
  const keyOf = (e) =>
...
export async function fetchFromKudaGo({ noCache = false } = {}) {
...
/* =========== Общие утилиты для JSON-LD провайдеров =========== */
...
export async function fetchFromYandexAfisha({ noCache = false } = {}) {
  const pages = [
  ...
  for (const url of pages) {
    ...
    for (const node of blocks) {
      for (const ev of flattenEvents(node)) {
        const mapped = mapJsonLdEvent(ev, { fallbackBase: "https://afisha.yandex.ru/moscow", source: "yandex-afisha" });
...
export async function fetchFromTicketland({ noCache = false } = {}) {
  const pages = [
  ...
  for (const url of pages) {
    ...
    for (const node of blocks) {
      for (const ev of flattenEvents(node)) {
        const mapped = mapJsonLdEvent(ev, { fallbackBase: "https://www.ticketland.ru/msk/", source: "ticketland" });
        if (mapped && looksLikeMoscow(mapped)) out.push(mapped);
      }
    }
  }
...
export async function fetchFromKassir({ noCache = false } = {}) {
  const pages = [
  ...
  for (const url of pages) {
    ...
    for (const node of blocks) {
      for (const ev of flattenEvents(node)) {
        const mapped = mapJsonLdEvent(ev, { fallbackBase: "https://msk.kassir.ru/teatr", source: "kassir" });
        if (mapped && looksLikeMoscow(mapped)) out.push(mapped);
      }
    }
  }
