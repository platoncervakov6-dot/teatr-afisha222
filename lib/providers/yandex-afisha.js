...
/**
 * Яндекс.Афиша (Москва): парсинг JSON-LD со страниц рубрик.
 * Мы читаем несколько наиболее плотных страниц.
 */
  ...
  for (const url of pages) {
    ...
    for (const node of blocks) {
      ...
      for (const ev of events) {
        const mapped = mapJsonLdEvent(ev, "yandex");
...
function mapJsonLdEvent(obj, source) {
  try {
    if (!obj || (obj["@type"] || obj.type) && !String(obj["@type"] || obj.type).toLowerCase().includes("event"))
      return null;
    ...
    const endDate = toISO(obj.endDate || obj.end_time || null);
...
function extractJsonLd(html) {
  ...
  while ((m = re.exec(html))) {
    const raw = m[1].trim();
    try {
      const json = JSON.parse(unescapeHtml(raw));
      blocks.push(json);
    } catch {
      // иногда внутри несколько JSON подряд — пробуем почленно
      const parts = safeSplitMultiJson(raw);
...
function safeSplitMultiJson(s) {
  // На случай когда в теге подряд несколько { ... }{ ... }
  const parts = [];
  let depth = 0, start = -1;
  for (let i=0;i<s.length;i++){
    const ch = s[i];
    if (ch === "{") { if (depth===0) start=i; depth++; }
    else if (ch === "}") { depth--; if (depth===0 && start>=0){ parts.push(s.slice(start, i+1)); start=-1; } }
  }
  return parts;
}
function hash(str) {
...
function dedupe(list) {
  ...
  for (const e of list) {
    const k = `${e.title}|${e.venue?.name}|${(e.dateStart||"").slice(0,10)}`.toLowerCase();
