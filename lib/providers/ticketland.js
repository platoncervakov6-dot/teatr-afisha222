// lib/providers/ticketland.js
import { normalizeText } from "../utils";

/**
 * Ticketland (Москва): лёгкий скрейп JSON-LD с основных разделов.
 * Если сайт прикрыт антиботом на конкретной странице — просто пропустим.
 */
export async function fetchFromTicketland({ city = "moscow", noCache = false } = {}) {
  const pages = [
  ...
  for (const url of pages) {
    ...
    for (const node of blocks) {
      ...
      for (const ev of events) {
        const mapped = mapJsonLdEvent(ev, "ticketland");
...
function mapJsonLdEvent(obj, source) {
  try {
    ...
    return {
      id: `ticketland:${hash(url || name + startDate)}`,
      source: "ticketland",
      sourceId: url || name,
      title: name,
      ...
      siteUrl: url || "https://www.ticketland.ru/msk/",
      buyUrl: (offers && offers.url) ? offers.url : (url || "https://www.ticketland.ru/msk/"),
...
function guessCategories(name, obj) {
  ...
  if (/(балет|balet|ballet)/.test(s)) cats.add("Балет");
